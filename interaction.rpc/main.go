...
func main() {
	...
	svr := first.NewServer(new(InteractionServiceImpl),
		server.WithServerBasicInfo(&rpcinfo.EndpointBasicInfo{ServiceName: "interaction.rpc"}),
		server.WithRegistry(registry),
		server.WithServiceAddr(addr{"tcp", "127.0.0.1:8891"}),
		//server.WithServiceAddr(addr{"tcp", "127.0.0.1:9891"}),
	)
	err = svr.Run()
	if err != nil {
		log.Fatal(err.Error())
