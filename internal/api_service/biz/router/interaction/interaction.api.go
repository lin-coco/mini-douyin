...
import (
	...
	"mini-douyin/internal/api_service/biz/handler/interaction"
)
	...
	root := r.Group("/", rootMw()...)
	{
		_douyin := root.Group("/douyin", _douyinMw()...)
		{
			_comment := _douyin.Group("/comment", _commentMw()...)
			{
				...
				_action.POST("/", append(_commentactionMw(), interaction.CommentAction)...)
			}
			{
		...
		}
		{
			_favorite := _douyin.Group("/favorite", _favoriteMw()...)
			{
				...
				_action0.POST("/", append(_favoriteactionMw(), interaction.FavoriteAction)...)
			}
			{
