...
import (
	...
	"mini-douyin/internal/api_service/biz/handler/society"
)
	...
	root := r.Group("/", rootMw()...)
	{
		_douyin := root.Group("/douyin", _douyinMw()...)
		{
			_message := _douyin.Group("/message", _messageMw()...)
			{
				...
				_action.POST("/", append(_messageactionMw(), society.MessageAction)...)
			}
			{
				_chat := _message.Group("/chat", _chatMw()...)
				_chat.GET("/", append(_messagechatMw(), society.MessageChat)...)
			}
		}
		{
			_relation := _douyin.Group("/relation", _relationMw()...)
			{
				...
				_action0.POST("/", append(_relationactionMw(), society.RelationAction)...)
			}
			{
				_follow := _relation.Group("/follow", _followMw()...)
				{
					_list := _follow.Group("/list", _listMw()...)
					_list.GET("/", append(_relationfollowlistMw(), society.RelationFollowList)...)
				}
			}
			{
				_follower := _relation.Group("/follower", _followerMw()...)
				{
					...
					_list0.GET("/", append(_relationfollowerlistMw(), society.RelationFollowerList)...)
				}
			}
			{
				_friend := _relation.Group("/friend", _friendMw()...)
				{
					...
					_list1.GET("/", append(_relationfriendlistMw(), society.RelationFriendList)...)
				}
			}
		}
	}
