...
func FeedRequest(ctx context.Context, c *app.RequestContext) {
	var err error
	var req HttpBasics.DouyinFeedRequest
	err = c.BindAndValidate(&req)
	resp := new(HttpBasics.DouyinFeedResponse)
	if err != nil {
	...
	res, err := api_service.BasicsRpcClient.GetVideo(ctx, &RpcBasics.GetVideoRequest{LatestTime: *latestTime})
	if err != nil {
		//log.Printf("BasicsRpcClient failed err:%v", err)
		hlog.Infof("BasicsRpcClient failed err:%v\n", err)
		c.JSON(consts.StatusBadRequest, returnErrorResponse(consts.StatusBadRequest, err.Error()))
		return
	}

	videoList := make([]*HttpBasics.Video, 0, len(res.VideoList))
	for _, video := range res.VideoList {
		...
		if isLogin {
			...
			societyInfoResp, err = api_service.SocietyRpcClient.SocietyInfo(ctx, &society.SocietyInfoRequest{MyId: claims.UserId, UserId: video.User.Id})
			if err != nil {
				//log.Printf("SocietyRpcClient failed err:%v", err)
			...
			societyInfoResp, err = api_service.SocietyRpcClient.SocietyInfo(ctx, &society.SocietyInfoRequest{MyId: video.User.Id, UserId: video.User.Id})
			if err != nil {
				//log.Printf("SocietyRpcClient failed err:%v", err)
				hlog.Infof("SocietyRpcClient failed err:%v\n", err)
				c.JSON(consts.StatusBadRequest, returnErrorResponse(consts.StatusBadRequest, err.Error()))
		...
		favoriteCountResp, err := api_service.InteractionRpcClient.GetVideoFavoriteCount(ctx, &interaction.GetVideoFavoriteCountRequest{VideoId: video.Id})
		if err != nil {
			hlog.Infof("InteractionRpcClient failed err:%v\n", err)
			c.JSON(consts.StatusBadRequest, returnErrorResponse(consts.StatusBadRequest, err.Error()))
			return
		}
		commentCountResp, err := api_service.InteractionRpcClient.GetVideoCommentCount(ctx, &interaction.GetVideoCommentCountRequest{VideoId: video.Id})
		if err != nil {
		...
		if isLogin {
			...
			isFavoriteResp, err := api_service.InteractionRpcClient.IsFavorite(ctx, &interaction.IsFavoriteRequest{UserId: claims.UserId, VideoId: video.Id})
			if err != nil {
				hlog.Infof("InteractionRpcClient failed err:%v", err)
				c.JSON(consts.StatusBadRequest, returnErrorResponse(consts.StatusBadRequest, err.Error()))
				return
		...
		videoList = append(videoList, &HttpBasics.Video{
			Id: video.Id,
			Author: &HttpBasics.User{
				Id:            video.User.Id,
...
func RegisterRequest(ctx context.Context, c *app.RequestContext) {
	var err error
	var req HttpBasics.DouyinUserRegisterRequest
	err = c.BindAndValidate(&req)
	if err != nil {
		c.JSON(consts.StatusBadRequest, returnErrorResponse(consts.StatusBadRequest, err.Error()))
		return
	}
	resp := new(HttpBasics.DouyinUserRegisterResponse)
	...
	res, err := api_service.BasicsRpcClient.CreateUser(ctx, &RpcBasics.CreateUserRequest{Username: req.Username, Password: req.Password})
	if err != nil {
...
func LoginRequest(ctx context.Context, c *app.RequestContext) {
	var err error
	var req HttpBasics.DouyinUserLoginRequest
	err = c.BindAndValidate(&req)
	if err != nil {
		c.JSON(consts.StatusBadRequest, returnErrorResponse(consts.StatusBadRequest, err.Error()))
		return
	...
	resp := new(HttpBasics.DouyinUserLoginResponse)
	if req.Username == "" || req.Password == "" {
	...
	res, err := api_service.BasicsRpcClient.CheckUser(ctx, &RpcBasics.CheckUserRequest{Username: req.Username, Password: req.Password})
	if err != nil {
		//log.Printf("BasicsRpcClient failed err:%v", err)
		hlog.Infof("BasicsRpcClient failed err:%v", err)
		c.JSON(consts.StatusBadRequest, returnErrorResponse(consts.StatusBadRequest, err.Error()))
		return
	}
	resp.StatusCode = 0
...
func UserRequest(ctx context.Context, c *app.RequestContext) {
	var err error
	var req HttpBasics.DouyinUserRequest
	err = c.BindAndValidate(&req)
	if err != nil {
		c.JSON(consts.StatusBadRequest, returnErrorResponse(consts.StatusBadRequest, err.Error()))
		return
	...
	resp := new(HttpBasics.DouyinUserResponse)
	...
	res, err := api_service.BasicsRpcClient.GetUserInfoById(ctx, &RpcBasics.GetUserRequest{UserId: userId})
	if err != nil {
		resp.StatusCode = 400
		resp.StatusMsg = new(string)
	...
	res2, err := api_service.SocietyRpcClient.SocietyInfo(ctx, &society.SocietyInfoRequest{MyId: myId, UserId: userId})
	if err != nil {
		resp.StatusCode = 400
		resp.StatusMsg = new(string)
		*resp.StatusMsg = err.Error()
	...
	GetInteractionInfoResponse, err := api_service.InteractionRpcClient.GetInteractionInfo(ctx, &interaction.GetInteractionInfoRequest{UserId: userId})
	if err != nil {
		resp.StatusCode = 400
		resp.StatusMsg = new(string)
		*resp.StatusMsg = err.Error()
...
func PublishActionRequest(ctx context.Context, c *app.RequestContext) {
	var err error
	var req HttpBasics.DouyinPublishActionRequest
	//err = c.BindAndValidate(&req)
	...
	title := req.Title
	data := req.Data
	resp := new(HttpBasics.DouyinPublishActionResponse)
	...
	_, err = api_service.BasicsRpcClient.UploadVideo(ctx, &RpcBasics.UploadVideoRequest{UserId: myId, Data: data, Title: title})
	if err != nil {
		hlog.Infof("BasicsRpcClient failed err:%v\n", err)
		c.JSON(consts.StatusBadRequest, returnErrorResponse(consts.StatusBadRequest, err.Error()))
		return
...
func PublishListRequest(ctx context.Context, c *app.RequestContext) {
	var err error
	var req HttpBasics.DouyinPublishListRequest
	err = c.BindAndValidate(&req)
	if err != nil {
		c.JSON(consts.StatusBadRequest, returnErrorResponse(consts.StatusBadRequest, err.Error()))
		return
	...
	userId := req.UserId
	myId := c.GetInt64("myId")
	resp := new(HttpBasics.DouyinPublishListResponse)
	if userId == 0 {
		hlog.Infof("userId is null")
		c.JSON(consts.StatusBadRequest, returnErrorResponse(consts.StatusBadRequest, "userId is null"))
		return
	}
	videosByUserIdResp, err := api_service.BasicsRpcClient.GetVideosByUserId(ctx, &RpcBasics.GetVideosByUserIdRequest{UserId: userId})
	if err != nil {
		hlog.Infof("BasicsRpcClient failed err:%v\n", err)
		c.JSON(consts.StatusBadRequest, returnErrorResponse(consts.StatusBadRequest, err.Error()))
		return
	...
	videos := make([]*HttpBasics.Video, 0, len(videoList))
	for _, video := range videosByUserIdResp.VideoList {
		...
		videoFavoriteCountResponse, err := api_service.InteractionRpcClient.GetVideoFavoriteCount(ctx, &interaction.GetVideoFavoriteCountRequest{VideoId: video.Id})
		if err != nil {
			hlog.Infof("InteractionRpcClient failed err:%v\n", err)
			c.JSON(consts.StatusBadRequest, returnErrorResponse(consts.StatusBadRequest, err.Error()))
			return
		}
		videoCommentCountResponse, err := api_service.InteractionRpcClient.GetVideoCommentCount(ctx, &interaction.GetVideoCommentCountRequest{VideoId: video.Id})
		if err != nil {
			hlog.Infof("InteractionRpcClient failed err:%v\n", err)
			c.JSON(consts.StatusBadRequest, returnErrorResponse(consts.StatusBadRequest, err.Error()))
			return
		...
		isFavoriteResponse, err := api_service.InteractionRpcClient.IsFavorite(ctx, &interaction.IsFavoriteRequest{UserId: myId})
		if err != nil {
			hlog.Infof("InteractionRpcClient failed err:%v\n", err)
			c.JSON(consts.StatusBadRequest, returnErrorResponse(consts.StatusBadRequest, err.Error()))
			return
		...
		societyInfoResponse, err := api_service.SocietyRpcClient.SocietyInfo(ctx, &society.SocietyInfoRequest{MyId: myId, UserId: userId})
		if err != nil {
			hlog.Infof("SocietyRpcClient failed err:%v\n", err)
			c.JSON(consts.StatusBadRequest, returnErrorResponse(consts.StatusBadRequest, err.Error()))
			return
		...
		videos = append(videos, &HttpBasics.Video{
			Id: video.Id,
			Author: &HttpBasics.User{
				Id:            video.User.Id,
				Name:          video.User.Name,
